...
class AdminDashboard(QWidget):
    ...
    def open_events_menu(self):
        ...
        button_open_feeging = QPushButton("Открытые кормления", self)
        button_open_feeging.clicked.connect(self.on_button_open_feeding_clicked)
        button_open_feeging.setStyleSheet("background-color: white; color: black;")

        layout = QVBoxLayout()
        layout.addWidget(button_excursions)
        layout.addWidget(button_exhibitions)
        layout.addWidget(button_open_feeging)
    ...
    def on_button_excursions_clicked(self):
        ...
        excursions_table.setColumnCount(3)
        excursions_table.setHorizontalHeaderLabels(
            ["Дата", "Время", "Название экскурсии"]
        )
        db = SessionLocal()
    ...
    def on_button_exhibitions_clicked(self):
        ...
        exhibitions_table.setColumnCount(3)
        exhibitions_table.setHorizontalHeaderLabels(
            ["Дата", "Время", "Название выставки"]
        )
        db = SessionLocal()
    ...
    def on_button_open_feeding_clicked(self):
        self.open_feeding_window = QWidget()
        self.open_feeding_window.setWindowTitle("Открытые кормления")
        self.open_feeding_window.resize(800, 600)

        open_feeding_table = QTableWidget()
        open_feeding_table.setColumnCount(3)
        open_feeding_table.setHorizontalHeaderLabels(
            ["Дата", "Время", "Кормление"]
        )
        db = SessionLocal()
        open_feeding_services = OpenFeedingService(db)
        open_feeding = open_feeding_services.get_all_open_feedings()
        db.close()

        open_feeding_table.setRowCount(len(open_feeding))

        for row, exhibition in enumerate(open_feeding):
            open_feeding_table.setItem(row, 0, QTableWidgetItem(str(exhibition.events_date)))
            open_feeding_table.setItem(row, 1, QTableWidgetItem(str(exhibition.events_time)))
            open_feeding_table.setItem(row, 2, QTableWidgetItem(str(exhibition.events_name)))

        layout = QVBoxLayout()
        layout.addWidget(open_feeding_table)
        self.open_feeding_window.setLayout(layout)
        self.open_feeding_window.show()
    ...
    def open_animals_menu(self):
        ...
        for row, animal in enumerate(animals):
            self.animals_table.setItem(row, 0, QTableWidgetItem(animal.animals_name))
            self.animals_table.setItem(row, 1, QTableWidgetItem(animal.animals_type))
            self.animals_table.setItem(row, 2, QTableWidgetItem(str(animal.animals_type_quantity)))
            self.animals_table.setItem(row, 3, QTableWidgetItem(str(animal.animals_weight)))
            self.animals_table.setItem(row, 4, QTableWidgetItem(animal.animals_color))
    ...
    def open_employees_menu(self):
        ...
        for row, employee in enumerate(employees):
            self.employees_table.setItem(row, 0, QTableWidgetItem(employee.employees_position))
            self.employees_table.setItem(row, 1, QTableWidgetItem(employee.employees_name))
            self.employees_table.setItem(row, 2, QTableWidgetItem(employee.employees_last_name))
            self.employees_table.setItem(row, 3, QTableWidgetItem(employee.employees_sex))
            self.employees_table.setItem(row, 4, QTableWidgetItem(str(employee.employees_age)))
            self.employees_table.setItem(row, 5, QTableWidgetItem(employee.employees_education))
            self.employees_table.setItem(row, 6, QTableWidgetItem(str(employee.employees_phone_number)))
            self.employees_table.setItem(row, 7, QTableWidgetItem(str(employee.employees_experience)))
            self.employees_table.setItem(row, 8, QTableWidgetItem(str(employee.employees_login)))
            self.employees_table.setItem(row, 9, QTableWidgetItem(str(employee.employees_password)))
