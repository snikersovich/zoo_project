...
class EmployeeService:
    ...
    def add_employee(self, position: str, name: str, last_name: str, sex: str, age: int, education: str, phone_number: int,
                     experience: int, login: int, password: int) -> Employees:
    ...
    def get_employee(self, employees_id: int) -> Employees:
        """Выборка сотрудника из базы данных по ID."""
        employee = self.db.query(Employees).filter(Employees.employees_id == employees_id).first()
        if not employee:
